 ...
 * 
 * Returns an array of objects of the form { baseName, }
 * 
 * @param {File[]} rawFiles 
 */
function analyzeAndWrapFiles(rawFiles, parseProgress) {
  ...
  const wrappedFiles = rawFiles.map((raw) => {
  ...
  if (numParents > 1) {
    parseProgress.warn("More than one parent log - is that what you want?");
  }
  if (numParents == 0 && numChildren > 1) {
    parseProgress.warn("Loading orphan child logs - is that what you want?");
  }
}
